...
bool PerfCounterMuninNodePlugin::OpenCounter()
{
    ...
    if (!m_CounterNames.empty()) {
        if (g_Config.GetValueB(m_SectionName, "TotalOnly", true)) {
            while (m_CounterNames.size() > 1) {
                m_CounterNames.erase(m_CounterNames.begin());
            }
        }
        else if (g_Config.GetValueB(m_SectionName, "DropTotal", true)) {
